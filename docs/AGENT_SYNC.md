...
| I1 | **Train nhánh B thật (8 epoch, GPU).** ADR-0020 đã triển khai và verify (1 epoch smoke-test: macro-F1 0.375 > nhánh A 0.191, đúng hướng). Trước khi chạy: `nvidia-smi` — nếu VRAM đã dùng > 6 GB, đợi 2-3 phút rồi thử lại (Claude có thể đang chạy song song trên cùng GPU). Lệnh: `.venv/Scripts/python.exe -m scripts.train_sed --encoder panns --audioset-checkpoint "artifacts/checkpoints/Cnn14_mAP=0.431.pth" --evaluate-test`. Ghi lại `run_id` in ra (dòng `"run": "ml/runs/sed_polyphonic_<timestamp>"`) vào nhật ký §4 — các task sau cần đúng ID này | Codex | `ml/runs/` | ADR-0020 ✅ | TODO |
| I2 | **Train nhánh C thật (8 epoch, GPU), sau khi I1 xong** (tránh tranh GPU với chính I1). Lệnh: `.venv/Scripts/python.exe -m scripts.train_sed --encoder panns --datasec-checkpoint ml/runs/classifier_datasec_20260923T121808Z/checkpoints/best.pt --evaluate-test`. Ghi `run_id` vào §4 | Codex | `ml/runs/` | I1 | TODO |
| I3 | **Đánh giá đầy đủ nhánh B**: `.venv/Scripts/python.exe -m scripts.sweep_threshold ml/runs/<run_id_B>` rồi `.venv/Scripts/python.exe -m scripts.evaluate_run ml/runs/<run_id_B>`. Cả hai script đã verify chạy đúng thật (H2) — nếu lỗi, đó là phát hiện thật cần báo ở §6, không phải giả định do thiếu code | Codex | `ml/runs/`, `docs/measurements/` | I1 | TODO |
| I4 | **Đánh giá đầy đủ nhánh C** — giống I3, đổi `<run_id_C>` | Codex | `ml/runs/`, `docs/measurements/` | I2 | TODO |
| I5 | **Tính `Δ = C − B` và `Δ = C − A`** (RQ1, ADR-0002) từ 3 file `evaluation.json` (A do Claude chạy, B/C từ I3/I4) — so `event_based_f1.f_measure` và cả hai `psds` (psds_1, psds_2). Viết `docs/measurements/rq1_delta_20260924.md` bằng script mới (đọc 3 JSON, không tính lại metric). **Đây là số thăm dò lần đầu** (1 seed, chưa qua D2-style seed check) — ghi rõ "chưa khoá chính thức", không tuyên bố kết luận RQ1 cuối cùng | Codex | `scripts/` (script mới), `docs/measurements/` | I3, I4, (nhánh A từ Claude) | TODO |
| I6 | **Sửa nhãn sai đã treo từ trước**: `report_per_class_metrics.py` dòng in "Best checkpoint SHA-256" đang lấy `manifest["config"]["checkpoint_sha256"]` — đó là hash checkpoint AudioSet **nguồn**, không phải `best.pt`. Xem [MỞ]/[ĐÓNG] cũ ở §6 đã có đề xuất sửa. Việc nhỏ, làm khi rảnh giữa các lần chờ train | Codex | `scripts/report_per_class_metrics.py` | — | TODO |
| I7 | **Dự phòng nếu xong hết I1-I6 mà vẫn còn thời gian:** lặp lại I1 với `--seed` khác (vd 1) cho cả nhánh B và C, so `event_based_f1`/`psds` giữa 2 seed — bằng chứng biến thiên giữa các lần chạy trước khi coi số ở I5 là ổn định. Không bắt buộc, không chặn gì | Codex | `ml/runs/` | I5 | TODO |
