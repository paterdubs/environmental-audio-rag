...
| I8 | **Khoá RQ1 chính thức: chạy lại nhánh B và C (seed=0, giống I1/I2 hệt lệnh) trên tree SẠCH** (`git status` rỗng trước khi chạy — nhánh A (I0) đã làm đúng cách này). Sau khi có `git.dirty=false` cho cả hai: chạy lại `sweep_threshold`+`evaluate_run` (như I3/I4), rồi `scripts.report_rq1_delta` với 3 run_id mới (A giữ nguyên `sed_polyphonic_20260923T173234Z`). **Không cần chạy lại A** — A đã `git.dirty=false` từ I0. Kiểm tra `git status` rỗng **ngay trước khi bấm chạy**, không tin vào "chắc là sạch". **Claude nhận task này** (người dùng duyệt, Codex đang nghỉ) — GPU rảnh (113 MiB/1%) | Claude | `ml/runs/`, `docs/measurements/` | I7 ✅ | 🔒 claude |
