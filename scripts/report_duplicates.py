...
def build_report(audit: dict, calibration: dict, stats: dict) -> str:
    ...
    return "\n".join(
        lines + alarm_section(audit) + calibration_section(calibration) + coverage_section(audit)
    )
...
def main() -> None:
    ...
    destination.write_text(build_report(audit, calibration, stats) + "\n", encoding="utf-8")
